...
export class RateLimitService {
   ...
   * Block IP address
   */
  private blockIP(ip: string, duration: number = this.BLOCK_DURATION): void {
  ...
  createApiKeyMiddleware() {
    return async (req: Request & { apiKey?: IApiKey }, res: Response, next: NextFunction) => {
      try {
        ...
        const endpointConfig = apiKey.rateLimits?.endpoints ? 
          apiKey.rateLimits.endpoints.get(endpoint) : undefined;
        ...
        if (!result.allowed) {
          ...
          await this.trackAbuse(clientIP, apiKey.keyPrefix);
   ...
   */
  createIPMiddleware(config: RateLimitConfig = RATE_LIMIT_DEFAULTS.GLOBAL) {
    return async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const result = await this.checkRateLimit(`ip:${clientIP}`, config);
        ...
        if (!result.allowed) {
          ...
          await this.trackAbuse(clientIP);
   ...
   */
  createEmailMiddleware(config: RateLimitConfig = {
    ...
    return async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const result = await this.checkRateLimit(`email:${email}`, config);
   ...
   */
  private async trackAbuse(ip: string, apiKeyPrefix?: string): Promise<void> {
    ...
    try {
      ...
      if (violations === 1) {
        await this.redis.expire(abuseKey, Math.ceil(abuseWindow / 1000));
      }
   ...
   */
  async getUsage(identifier: string, config: RateLimitConfig, endpoint?: string): Promise<{
    used: number;
    limit: number;
    remaining: number;
    resetAt: Date;
