...
export const refreshToken = withErrorHandling(async (req: Request, res: Response) => {
...
export const me = withErrorHandling(async (req: AuthenticatedRequest, res: Response) => {
...
export const changePassword = withErrorHandling(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.userId;
  if (userId === undefined || userId.trim().length === 0) {
    sendError(res, 'User authentication required', 401);
    return;
...
export const deleteAccount = withErrorHandling(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.userId;
  if (userId === undefined || userId.trim().length === 0) {
    sendError(res, 'User authentication required', 401);
    return;
...
// Admin methods
export const getAllUsers = withErrorHandling(async (req: Request, res: Response) => {
  const users = await User.find({}, '-password');
  sendSuccess(res, users);
});

export const deleteUser = withErrorHandling(async (req: Request, res: Response) => {
