...
const sendWithTwilio = async (message: SMSMessage, config: SMSConfig): Promise<SMSResult> => {
  try {
...
const sendWithSNS = async (message: SMSMessage, config: SMSConfig): Promise<SMSResult> => {
  try {
...
export const getSMSServiceStatus = async (): Promise<{
  provider: string;
  healthy: boolean;
  lastTest?: Date;
  error?: string;
  ...
  try {
    ...
    const isConfigValid = !!(config.fromNumber && (
      (config.provider === 'twilio' && config.accountSid && config.authToken) ||
      (config.provider === 'sns' && config.apiKey && config.region)
    ));
...
export const isOptedOut = async (phoneNumber: string): Promise<boolean> => {
  const normalizedNumber = normalizePhoneNumber(phoneNumber);
