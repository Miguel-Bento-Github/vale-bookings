...
// Create logger object using exported functions
const logger = {
  info: logInfo,
  warn: logWarning,
  error: logError
 ...
 * Handles API key validation, domain verification, and key rotation
 */
export class WidgetAuthService {
   ...
   */
  async validateApiKey(rawKey: string): Promise<IApiKeyWithMethods | null> {
    try {
  ...
  async validateRequest(req: Request): Promise<{
    ...
    const domain = this.extractOrigin(req);
    if (!domain) {
      return {
        isValid: false,
   ...
   * Rotate API key
   */
  async rotateApiKey(apiKeyId: string, createdBy: string): Promise<{
   ...
   * Revoke API key
   */
  async revokeApiKey(apiKeyId: string): Promise<IApiKeyWithMethods> {
